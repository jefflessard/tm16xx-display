 ...
 * Return: Number of bytes written or negative error code
 */
static int tm16xx_spi_write(struct tm16xx_display *display, u8 *data,
			    size_t len)
{
...
static int tm1628_init(struct tm16xx_display *display)
{
	...
	static const u8 DATA_ADDR_MODE = 0 << 2;
	static const u8 DATA_WRITE_MODE = 0 << 1 | 0 << 0;
	static const u8 CTRL_CMD = 1 << 7 | 0 << 6;
	static const u8 ON_FLAG = 1 << 3;
	static const u8 BR_MASK = 7, BR_SHIFT = 0;

	const enum led_brightness brightness = display->main_led.brightness;
	const u8 num_grids = display->transpose_display_data ?
				     DIGIT_SEGMENTS :
	...
	cmd = CTRL_CMD | ((brightness && 1) *
			  (((brightness - 1) & BR_MASK) << BR_SHIFT | ON_FLAG));
	ret = tm16xx_spi_write(display, &cmd, 1);
	if (ret < 0)
		return ret;
...
{
	static const u8 ON_FLAG = 1;
	static const u8 BR_MASK = 7, BR_SHIFT = 4;
	static const u8 SEG8_MODE = 0 << 3;
	/* SEG7_MODE = 1 << 3
	...
	cmds[0] = 0x48;
	cmds[1] = (brightness && 1) *
		  ((brightness & BR_MASK) << BR_SHIFT | SEG8_MODE | ON_FLAG);
...
{
	static const u8 ON_FLAG = 1;
	static const u8 BR_MASK = 3, BR_SHIFT = 5;
	const enum led_brightness brightness = display->main_led.brightness;
	u8 cmds[2];

	cmds[0] = 0x48;
	cmds[1] = (brightness && 1) *
		  (((brightness % 3) & BR_MASK) << BR_SHIFT | ON_FLAG);
...
{
	static const u8 ON_FLAG = 1;
	static const u8 BR_MASK = 7, BR_SHIFT = 1;
	const enum led_brightness brightness = display->main_led.brightness;
	u8 cmds[2];
...
static void tm16xx_display_flush_data(struct work_struct *work)
{
	...
	if (display->controller->data) {
		for (i = 0; i < display->display_data_len; i++) {
			ret = display->controller->data(
				display, i, display->display_data[i]);
			if (ret < 0) {
				dev_err(display->dev,
			   ...
			   enum led_brightness value)
{
	struct tm16xx_led *led =
		container_of(led_cdev, struct tm16xx_led, cdev);
	struct tm16xx_display *display = dev_get_drvdata(led_cdev->dev->parent);
...
static DEVICE_ATTR(value, 0644, tm16xx_display_value_show,
		   tm16xx_display_value_store);
static DEVICE_ATTR(num_digits, 0444, tm16xx_num_digits_show, NULL);
static DEVICE_ATTR(segments, 0644, tm16xx_segment_mapping_show,
		   tm16xx_segment_mapping_store);
static DEVICE_ATTR(digits, 0644, tm16xx_digits_ordering_show,
		   tm16xx_digits_ordering_store);
static DEVICE_ATTR(map_seg7, 0644, tm16xx_map_seg7_show, tm16xx_map_seg7_store);

static struct attribute *tm16xx_main_led_attrs[] = {
	&dev_attr_value.attr,	 &dev_attr_num_digits.attr,
	&dev_attr_segments.attr, &dev_attr_digits.attr,
	&dev_attr_map_seg7.attr, NULL,
};
...
static int tm16xx_display_init(struct tm16xx_display *display)
{
	...
	if (default_value && strlen(default_value) > 0) {
		tm16xx_display_value_store(display->main_led.dev, NULL,
					   default_value,
...
static int tm16xx_parse_dt(struct device *dev, struct tm16xx_display *display)
{
	...
	digits = devm_kcalloc(dev, display->num_digits, sizeof(*digits),
			      GFP_KERNEL);
	if (!digits)
		return -ENOMEM;
	...
	if (ret < 0) {
		dev_err(dev,
			"Failed to read 'tm16xx,segment-mapping' property: %d\n",
			ret);
		return ret;
	}
	...
	for (i = 0; i < DIGIT_SEGMENTS; i++) {
		if (display->segment_mapping[i] < MIN_SEGMENT ||
			...
			dev_err(dev,
				...
				display->segment_mapping[i], MIN_SEGMENT,
				MAX_SEGMENT);
			return -EINVAL;
		}
...
static const struct spi_device_id tm16xx_spi_id[] = {
	{ "tm1618", 0 }, { "tm1620", 0 }, { "tm1628", 0 }, { "fd620", 0 },
	{ "fd628", 0 },	 { "pt6964", 0 }, { "hbs658", 0 }, { /* sentinel */ }
};
